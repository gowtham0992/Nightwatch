...
function ContractPreview({ draft, dataset, frozen }) {
  const rows = [
    ...
    ['Agent registry', draft.delegation ? `${draft.delegation.approved_agents.length} identities pinned` : 'Loading'],
    ['Deployment', 'Locked'],
  ];
...
export default function MissionBuilder({ onCancel, onLaunched }) {
  ...
  useEffect(() => { let ignore = false; getOperatorCapabilities().then((value) => { if (ignore) return; setCapabilities(value); const model = value.models[0]; const baseline = model.baseline_artifacts[0]; setDraft((current) => ({ ...current, model: { id: model.id, revision: model.revision }, baseline_artifact: baseline.id, instruction: baseline.instruction, delegation: { taxonomy_version: value.agent_fleet.taxonomy_version, maximum_specialists: value.agent_fleet.maximum_specialists, mandatory_specialists: value.agent_fleet.mandatory_specialists, approved_agents: value.agent_fleet.approved_agents } })); }).catch((reason) => !ignore && setError(reason.message)); return () => { ignore = true; }; }, []);
