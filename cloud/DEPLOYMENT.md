...
The canonical public judge URL is `https://nightwatch-public-w3a6oefsma-uc.a.run.app`. Commit `45775d4` is the current public and authenticated UI release. Revisions `nightwatch-public-hiddenproof-45775d4` and `nightwatch-evidence-hiddenproof-45775d4` run immutable service image digest `sha256:ffffb000f00084aa1bb6e332f38af05ddfd6161c5523c9ebd2e050a9c06679d0`. The verifiers, mission control, stage worker, and specialist services remain on the hardened `3ccbd0b` release described below. The judge view defaults to the adaptive-fleet refusal mission and exposes only its bundled, strictly redacted evidence.
...
## Hidden-regression evidence release — August 29, 2026 UTC

Commit `45775d4` corrects the retained hidden-regression presentation without changing frozen evidence or release policy. The judge dossier now reconstructs all five recorded invariants: four headline checks pass, while routine-message recall falls from 87.5% to 75.0% and fails the protected baseline. The same projection identifies the retained mission as a pre-registry bounded-curriculum run instead of retroactively describing it as Agent Registry/A2A orchestration. The current adaptive-fleet mission retains its Registry/A2A receipts, and the qualified comparison retains its four passing checks.

GitHub Actions run `33237678309` passed 290 Python tests, 32 frontend tests, production dependency verification, the Vite production build, and all three Docker builds. Cloud Build `384e7ea9-770e-436f-9730-6c99fee20622` produced immutable service digest `sha256:ffffb000f00084aa1bb6e332f38af05ddfd6161c5523c9ebd2e050a9c06679d0`. Public revision `nightwatch-public-hiddenproof-45775d4` and authenticated revision `nightwatch-evidence-hiddenproof-45775d4` were exercised at zero traffic before promotion. Canonical production then reproduced the five-check refusal, the current Registry/A2A refusal, and the qualified comparison with clean browser consoles. Both health contracts report release `45775d4`, and neither revision has ERROR-level logs.
