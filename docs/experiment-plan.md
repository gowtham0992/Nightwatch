...
The retained Modal capacity run `ap-i0FFgxHmq7N7OYIGjtCXuC` was also **refused**. It improved regression to 71/80 (88.75%), preserved both non-page recalls, produced complete coverage, and had no critical miss, but safety remained 25/30 (83.3%) and target accuracy rose to 26/40 (65%), exceeding the withheld-behavior ceiling. Capacity alone therefore did not qualify the student. Stop GPU iteration and reassess the v0 experiment before authorizing any further curriculum or configuration change; do not weaken either threshold.

The reassessment is governed by [evidence audit protocol v1](evidence-audit-protocol.md). It audits all 300 development and frozen cases prediction-blind, retains every policy-v1 artifact, and requires explicit adjudication before any versioned evidence change. Policy v2 removes only the artificial target ceiling; all safety, regression, recall, critical-miss, and coverage requirements remain unchanged.

## Adjudicated policy-v2 result

The pinned Gemma 3 1B candidate is qualified for deployment under policy v2 on the adjudicated development evidence. It scores 69/80 regression (86.25%), 28/30 safety (93.3%), and 28/40 target (70%); regression recall is 83.3% for `defer` and 75% for `investigate`, with complete prediction coverage and zero critical safety misses. This is a governed qualification result, not evidence of general model safety or a claim that a deployed model was repaired.

The result followed the frozen protocol without additional training or inference. Gemini's prediction-blind pass agreed with 256 of 300 retained labels and sent 44 disagreements to an independent adjudicator, who upheld 23 and changed 21. The same frozen prediction files were then scored once against versioned v2 evidence. The original generative 270M candidate remained refused, and the safety-augmented 270M candidate remained refused at 86.7% safety, below the unchanged 90% floor. The 1B candidate was the only retained candidate to pass every policy-v2 invariant.
